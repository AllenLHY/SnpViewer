    ...
    try:
        ...
        for i in range(n_ports):
            for j in range(n_ports):
                ...
                data_dict[f'{param_name}_real'] = np.real(s_param)
                data_dict[f'{param_name}_imag'] = np.imag(s_param)
...
def get_data_types(df, param_type):
    ...
    for col in cols:
        if col.startswith(param_type + '_'):
            data_type = col.split('_')[1]
            if data_type not in ('real', 'imag'):
                data_types.add(data_type)
    if f'{param_type}_real' in cols and f'{param_type}_imag' in cols:
    ...
    if files_data:
        ...
        if visible_files:
            ...
            for idx, (filename, file_info) in enumerate(visible_files.items()):
                ...
                if is_smith:
                    # Scattersmith expects normalized impedance z = Z/Z0, not Γ
                    # Convert: z = (1 + Γ) / (1 - Γ)
                    gamma = (df_filtered[f'{selected_param}_real'].values
                             + 1j * df_filtered[f'{selected_param}_imag'].values)
                    z_norm = (1 + gamma) / (1 - gamma)
                    fig.add_trace(go.Scattersmith(
                        real=np.real(z_norm).tolist(),
                        imag=np.imag(z_norm).tolist(),
                        customdata=df_filtered['Frequency'].tolist(),
                        mode='lines',
                        name=display_name,
            ...
            if is_smith:
                ...
                if markers_list:
                    ...
                    legend_height = num_legend_items * 21.5 + 100
